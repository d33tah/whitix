...
static char* VtGetName(char* name)
{
	sprintf(path, "/System/Devices/Consoles/%s", name);	
	return path;
}

static int GetVirtualTerminal(void)
{
    int master;
    char name[64];
    
    if (SysConfRead("/Devices/Consoles/VirtualMaster/newTerminal", name, 64) < 0)
    {
    	printf("Could not create new virtual terminal\n");
    	return 1;
    }
    
	ptydev = VtGetName(name);

	master = SysOpen(ptydev, _SYS_FILE_READ | _SYS_FILE_WRITE, 0);
...
static int term (s_window_t *window)
{
	char* args[]={""};
	s_pollfd_t *pfd;
	...
	SysCreateProcess("/Applications/burn", fds, args);
...
int main (int argc, char *argv[])
{
	...
	s_window_set_title(window, "Blaze terminal");
	...
	if (s_font_init(&font, "veramono.ttf") == -1)
	{
		printf("term: could not load %s\n", "veramono.ttf");
		return 0;
	}
