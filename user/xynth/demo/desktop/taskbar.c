...
void taskbar_progs_draw_client (tbar_progs_t *tbar_progs, s_surface_t *surface, s_desktop_client_t *client, int x, int y, int w, int h)
{
	...
	s_fillbox(surface, x, y, w, h, s_rgbcolor(surface, 56, 56, 56));
...
void taskbar_clock_draw (s_window_t *window, s_timer_t *timer)
{
	...
	int _w = 0;
        ...
        tbar_data_t *tbar_data;
        tbar_clock_t *tbar_clock;
	int c0 = s_rgbcolor(window->surface, 94, 94, 94);
	int c1 = s_rgbcolor(window->surface, 94, 94, 94);
	int c2 = s_rgbcolor(window->surface, 94, 94, 94);
...
void taskbar_start (s_window_t *window, s_config_t *cfg)
{
	...
	s_fillbox(window->surface, 0, 0, window->surface->buf->w, window->surface->buf->h, s_rgbcolor(window->surface, 0, 0, 0));
	s_fillbox(window->surface, 1, 1, window->surface->buf->w - 1, window->surface->buf->h - 1, s_rgbcolor(window->surface, 0, 0, 0));
