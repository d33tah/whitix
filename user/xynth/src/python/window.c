...
PyObject* p_window_set_back(PyObject* self, PyObject* args);

PyMethodDef p_window_methods[] = {
	{"setTitle", p_window_set_title, 1},
	{"show", p_window_show, 1},
	{"main", p_window_main, 1},
	{"formDraw", p_window_form_draw, 1},
	{"hide", p_window_hide, 1},
	{"setDimensions", p_window_set_coor, 1},
	{"setResizeable", p_window_set_resizeable, 1},
	{"setAlwaysOnTop", p_window_set_alwaysontop, 1},
	{"setBack", p_window_set_back, 1},
...
PyTypeObject p_window_type = {
	...
	"Window object, represents a Xynth window",         /* tp_doc */
	0,                         /* tp_traverse */
	0,                         /* tp_clear */
	0,                         /* tp_richcompare */
	0,                         /* tp_weaklistoffset */
...
PyObject * p_window_set_coor (PyObject *self, PyObject *args)
{
	int i[4];
	p_window_t *window = (p_window_t *) self;
	if (!PyArg_ParseTuple(args, "iiii:p_window_set_coor", &i[0], &i[1], &i[2], &i[3])) {
		return NULL;
	}
	s_window_set_coor(window->window, WINDOW_NOFORM, i[0], i[1], i[2], i[3]);
	return Py_BuildValue("i", 0);
}
...
PyObject* p_window_set_back(PyObject* self, PyObject* args)
{
	...
	if (!PyArg_ParseTuple(args, "iii:p_window_set_back", &r, &g, &b))
		return NULL;
		
	s_fillbox(window->window->surface, 0, 0, window->window->surface->width,
		window->window->surface->height, s_rgbcolor(window->window->surface, r, g, b));
	return Py_BuildValue("i", 0);
}
...
PyObject * wrap_window (PyObject *self, PyObject *args)
{
	...
	s_window_set_title(window->window, "Whitix application");	
