...
int BlazeServerConfig(s_server_conf_t* config)
{
	/* Open the registry and read the configuration methodically. */
	struct Registry registry;
	struct RegKeySet keySet;
	
	RegRegistryOpen(NULL, &registry);
	
	/* Video. */
	RegKeySetOpen(&registry, "/Applications/Xynth/Video", 0, &keySet);
	
	RegKeyReadString(&keySet, "driver", &config->general.driver, NULL);
	
	RegRegistryClose(&registry);
}
